...
do
	...
	awk '
		...
		## GROUP BY to sum descendants per individual Save 'id' as key and # offspring
		## as value in a hash.
		FNR == NR {
		...
		## JOIN: Process second file of arguments.
		## If first field is found in the hash, print offspring, otherwise 0
		FNR < NR {
			if ( $1 > 1 ) {
	...
	# Now, we have a single file with columns:
